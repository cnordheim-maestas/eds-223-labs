...
london_geom <- st_sfc(london_point, crs = 4326) # object to convert to geometry
...
```{r}
class(london_attrib) # data.frame
...
# check class
class(london_sf) # "sf" "data.frame"
```

We can also check out what the CRS looks like:

```{r}
st_crs(london_sf) # WGS 84, EPSG:4326

st_crs(london_sf)$IsGeographic # TRUE

st_crs(london_sf)$proj4string # "+proj=longlat +datum=WGS84 +no_defs"
```
...
```{r}
world <- spData::world
class(world) # "sf" "tbl_df" "tbl" "data.frame" sf tibble dataframe

dim(world) # dimmensions

colnames(world) # column names, geom column has the geographic data
```
...
```{r}
world_df <- world %>%
  dplyr::select(-geom) # doesn't actually remove the geom column
...
```{r}
st_crs(london_proj) # EPSG:27700; British National Grid
```
...
```{r}
london_sf_transform <- st_transform(london_sf, crs = st_crs(london_proj)) # transform the london_sf to have the same one as the object london_proj
```
...
tm_shape(world, crs = 3395) + # looks different!
  tm_fill(fill = "area_km2") 
...
```{r}
# area is less than 10k kilometers squared
world1 <- world %>%
  filter(area_km2 < 10000)
summary(world1$area_km2)

# rows where life expectancy is higher than 80
world2 <- world %>%
  filter(lifeExp >= 80)
nrow(world2)
```
...
world %>%
  ...
  select(name_long, continent, lifeExp) %>% # columns I care about
  slice_max(lifeExp) %>% # maximum of observation in this column
  st_drop_geometry() # remove the geometry, not relevant here
...
```{r}
world %>%
  st_drop_geometry() %>% # dont need this
  group_by(continent) %>% # this is the grouping level
  summarize(population = sum(pop, na.rm = TRUE), # total pop of eachcontinent
            area_km2 = sum(area_km2, na.rm = TRUE), # total area of each continent
            n_countries = n()) %>% # the number of countries (to arrange by)
  mutate(density = round(population/area_km2)) %>% # calculate the density (pop/area) for the continent
  slice_max(density, n = 7) %>% # get the top 7 maximum pop densities per continent
  arrange(desc(n_countries)) # arrange by the number of countries
```
