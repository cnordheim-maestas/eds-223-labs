...
https://eds-223-geospatial.github.io/course-materials/labs/week4.html
...
library(spDataLarge) # spatial data ## will not work with this version of R
...
nlyr(landsat) # test number of layers in raster object (4)
names(landsat) # names of layers
```
...
```{r}
summary(zion_elevation) # strm is the name of the raster layer
```
...
```{r}
freq(zion_land) # how many pixels or cells calssified as ...
```
...
# 3 past 465 so third observation in next row
zion_elevation[2,3]
```
...
```{r}
zion_elevation[1, 1] <- 0 # overwrite the cell with 0 for elevation
zion_elevation[1, 1]
...
```{r}
test_raster <- zion_elevation # make new raster to play with
# check na values
global(is.na(test_raster), "sum") # none
test_raster[test_raster < 20] <- NA
global(is.na(test_raster), "sum") # 1
```
...
id <- cellFromXY(zion_elevation, xy = point) # object I want to look for point in, xy = my coordinates
# index to cell
zion_elevation[id] # bc called it id above

# returned that the srtm is 2398

# approach 2
# extract raster values at point
terra::extract(zion_elevation, point) # defined the point as point above
...
clip <- rast(xmin = -113.3, xmax = -113, ymin = 37.2, ymax = 37.9,
            resolution = 0.3, # each cell is 0.3 x 0.3 degrees
            vals = 1) # each cell value of 1

# select values that fall within smaller extent
#zion_elevation_clip <- zion_elevation[clip] # bigger[clipped one]
...
```{r}
zion_elevation_clip <- zion_elevation[clip, drop = FALSE] # now it is a spatial raster!!
...
masked1 <- zion_elevation[rmask, drop = FALSE] # [masked_raster, drop = FALSE to keep geometry]  
# approach 2: mask() function
masked2 <- mask(zion_elevation, rmask)   #(whole raster, masked raster)  

## think of this like "filtering" in a sf
```
...
local: cell by cell (boolean & simple operations)

focal: aggregate neighboring cells, uses corresponding output

zonal: aggregate based on grouping variable

global: aggregate all cells
...
# the min of "min value: - ?" for log (0) is adorable

# create boolean true false for > 5
...
```{r}
# create reclassification matrix (rcl)
...
elevation_focal <- focal(zion_elevation, 
                         w = matrix(1, nrow = 9, ncol = 9), # create moving window, w is weight, I want all to be assigned equally
...
```{r}
zonal(zion_elevation, reclassified, fun = "mean") %>% # raster, grouping matrix, function
...
```{r}
elev_2 <- extend(zion_elevation, c(1, 200)) # add one row and 200 columns
# adds empty cells
```
...
```{r}
zion_elevation + elev_2 # Error: [+] extents do not match
```
...
```{r}
# decreasing the resolution of the raster
zion_elevation_coarse <- aggregate(zion_elevation, # what to aggregate
                                    fact = 5, # by a factor of 5
                                    fun = mean) # take the mean of the factor of 5 cells
```
...
zion_elevation_disagg <- disagg(zion_elevation_coarse, 
                                fact = 5, 
                                method = "bilinear") # 4 closest cells, giving equal weight and disaggregating from that
