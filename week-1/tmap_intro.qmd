...
# add raster of global elevations
world_elevation <- read_stars(here("week-1", "data", "worldelevation.tif")) # stars bc raster data # note this is a tif file

# add country boundaries
world_vector <- read_sf(here("week-1", "data", "worldvector.gpkg")) # geopackage

# add major cities
world_cities <- read_sf(here("week-1", "data", "worldcities.gpkg")) # geopackage
...
```{r}
# plotting two spatial objects, the second one appears on top of the first!
...
tm_shape(world_cities) +
  tm_dots() +
  tm_text("name", size = 0.7, col = "steelblue2") # the names of the cities that I have dots on, specifying the column I am using in the `world_cities` data
```
...
tm_shape(world_cities) + # plot world_cities first
  tm_dots() + # dots on the bottom, hard to see at all
  tm_text("name") + # then names, hard to see
tm_shape(world_elevation) + # elevation next
  tm_raster() +
tm_shape(world_vector) + # the countries of the world on top
  tm_borders() # with the borders on tippy top

## important: your first tm_shape will be the auto spatial extent
```
...
```{r}
tm_shape(world_elevation) + # elevation first
  tm_raster() + 
tm_shape(world_vector) +
  tm_borders() +
tm_shape(world_cities, is.main = TRUE) + # plot world_cities last, but set as main shape, so this will be the extent of the map!!! and the crs
  tm_dots() +
  tm_text("name")
...
```{r}
tm_shape(world_elevation, bbox = c(-15, 35, 45, 65)) + # add bounding box to restrict extent # always in coordinates of crs, Long lat coordinates (wgs84) assumed. 
  tm_raster() + # bc world_elevation is a raster
tm_shape(world_vector) +
  tm_borders() # borders bc raster data
```
...
```{r}
tm_shape(world_cities) +
  tm_symbols() # default is circles, centroid of city polygons
```
...
```{r}
tm_shape(world_cities) +
 # tm_squares() # one option
  tm_symbols(shape = 22) # shape number for a square, another option
...
tm_shape(world_vector) +
  tm_polygons(fill = "life_expectancy", 
              fill.scale = tm_scale(values = c("yellow", "darkgreen")), # update colors here
              fill.legend = tm_legend(title = "Life Expectancy (years)", ## update legend here
...
tm_shape(world_vector) +
  tm_polygons(fill = "life_expectancy", 
              fill.scale = tm_scale(values = "brewer.yl_gn"), # color brewer
              fill.legend = tm_legend(title = "Life Expectancy (years)"))
```
...
tm_shape(world_vector) +
  tm_polygons(fill = "lightblue") +
tm_shape(world_cities) +
  tm_symbols(size = "pop2020", ## now the size of the bubble is based on population data from 2020!
...
tm_shape(world_vector) +
  ...
  tm_layout(
    #text.fontfamily = "Montserrat",
            style= "beaver",
            #color_vision_deficiency_sim = ("protan"),
            bg.color = "grey95")
```

customize breaks in legend

```{r}
tm_shape(world_vector) +
  tm_polygons(fill = "gdp_per_cap", 
              fill.scale = tm_scale(breaks = c(0, 500, 2000, 5000, Inf),
                                    labels =c("low", "medium", "high", "very high"),
                                    values =  viridis(8)),
              fill.legend = tm_legend(title  = "GDP per cap"))
```
