...
tm_shape(nz_height) + # inputting the height data
  tm_dots(fill = "red") # dots bc they are points
```
...
c_height1 <- nz_height[canterbury, ] #subsets the nz_height to canterbury. remember rows,columns, to taking all columns, only rows matching canterbury
```
...
nrow(c_height_disjoint) ==nrow(nz_height)
```
...
```{r}
# subset to the features in Cantebury, using st_filter
c_height2 <- nz_height %>%
...
nz_height_sgbp <- st_intersects(x = nz_height, y = canterbury) # sparse geometry binary predicate 
nz_height_sgbp # empty if they dont intersect, 1 if they do

# convert to logical vector
nz_height_logical <- lengths(nz_height_sgbp) > 0

# filter based on logical vector
c_height3 = nz_height[nz_height_logical, ] # woohoo same 70 observations
```
...
nz_height_left_join <- st_join(nz_height, nz) %>%
  dplyr::select(id = t50_fid, elevation, region = Name) # new = old; region = Name
...
if(any(st_intersects(cycle_hire, cycle_hire_osm, sparse = FALSE)) == TRUE){
  print("points overlap") # every time they do intersect
} else{
  warning("points don't overlap") # every time they dont
}

# this is a probelm if we are trying to join the df's
# interactive plot
tmap_mode("view")
...
cycle_hire_join <- cycle_hire_join %>%
  group_by(id) %>% ## only want one capacity for each bike station, those that have more than 1, we will
  summarise(capacity = mean(capacity)) ## take the mean
...
aggregating_zones_area_weighted <- st_interpolate_aw(incongruent_2, 
                                                     aggregating_zones, # aggregate to this level
                                                     extensive = TRUE) # when working with population count or sums data, if using density or a rate, use extensive = false
```
